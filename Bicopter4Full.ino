...
void loop() {
  ...
  if (cmd.params[0] == 2) {
    ...
    if (nicla_flag != 0) {// checks if new yaw occurs
        ...
        if (move == false) { // Check if not in moving mode
         if (abs(x_cal - 0.5) < 0.16 && terms.y_strength != 0) { // makes sure yaw is in center before making height adjustments
          if (abs(y_cal - terms.y_thresh) > terms.h_ratio) {
            ...
            lastBalloonDetectionTime = micros(); // last time balloon detected
            behave.params[1] = terms.fx_togoal; // forward_force
        ...
        // forward_force = 0.13;
            ...
            // moving mode
            if (micros() - lastBalloonDetectionTime > TURNING_DELAY) {
                move = false;
                behave.params[1] = terms.fx_togoal; // forward force
            } 
            else { // move but fix height
                z_estimator = _height + terms.y_strength * (y_cal - terms.y_thresh);
                behave.params[1] = terms.fx_charge; // forward_force
            }
        }
        ...
        float yaw = senses[5] - 0.2; // fix angle of yaw
        if (yaw < -PI) {
            yaw = PI;
        }
        behave.params[1] = 0.12; // forward_force
        behave.params[4] = yaw; // angle of yaw
      } 
